...
function android_studio_parser(project) {
    if (!fs.existsSync(path.join(project, 'app', 'src', 'AndroidManifest.xml'))) {
        throw new CordovaError('The provided path "' + project + '" is not an Android studio project.');
    }
    ...
    this.path = project;
    this.strings = path.join(this.path, 'app', 'src', 'res', 'values', 'strings.xml');
    this.manifest = path.join(this.path, 'app', 'src', 'AndroidManifest.xml');
    this.android_config = path.join(this.path, 'app','src', 'res', 'xml', 'config.xml');
}
